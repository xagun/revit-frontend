...
class HomeForm extends Component {
  ...
  componentDidMount() {
    if (this.props.match.params.id) {
      this.getHomeDetails(this.props.match.params.id);
    }
  }

  getHomeDetails = async (id) => {
    let admin = JSON.parse(localStorage.getItem("admin"));
    try {
      const response = await Axios.get(`${BASE_URL}/homenews/${id}`);
    ...
    try {
      ...
      if (this.props.match.params.id) {
        const updateResponse = await Axios.put(
          `${BASE_URL}/homenews/update/${this.props.match.params.id}`,
          fileData,
          {
            headers: {
